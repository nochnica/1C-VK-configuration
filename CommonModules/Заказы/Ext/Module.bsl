...
Функция ПолучитьТекстЗаказов(from_id) Экспорт 
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Для каждого Стр Из ТабЧасть Цикл
            ...
            Кол   = Стр.Количество;
            Цена  = ?(ТипЗнч(Стр.Цена)=Тип("Число"), Стр.Цена, 0);
            Подит = Цена * Кол;
...
Процедура ОбработатьСтрокуКорзины(Результат, Док)
	...
	Если СсылкаНом <> Неопределено Тогда
        ...
        Строка.Цена = ЦенаТовара;   
		  ...
		  // Отправляем себе тестовое сообщение — увидим, что отдала функция
	    VK.message_send(
	      Формат(0, "ЧРГ=''; ЧГ=0"), 552360884,                   
	      "DEBUG: для номенклатуры «" + СсылкаНом.Наименование + "» цена = " + Строка(ЦенаТовара)
	    );
	КонецЕсли;	
КонецПроцедуры
