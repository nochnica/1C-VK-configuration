...
Функция СформироватьКлавиатуру(text = "", level = 0, from_id = "") Экспорт 
	...
	Если text = "catalog" ИЛИ text = "Начать" Тогда
	    message = "КАТАДЛШ БЛЯТЬ ачали работу с этим ботом." + Символы.ПС + "Это бот-продавец товаров."; 
		template = СформироватьКарусель();
		//buttons = СформироватьГлавноеМеню(); 
		//message = "В нашем магазине есть такие товары!";
		ДанныеВозврата.Вставить("template", template);
	ИначеЕсли text = "catalog" Тогда
		template = СформироватьКарусель();
		buttons = СформироватьГлавноеМеню(); 
		message = "В нашем магазине есть такие товары!";
		ДанныеВозврата.Вставить("template", template);
...
Функция СформироватьЭлементыКарусели()
	...
	Элементы.Добавить(СоздатьЭлемент("Заголовок 1", "Описание 1", , "Кнопка 1"));
	Элементы.Добавить(СоздатьЭлемент("Заголовок 2","Описание 2", , "Кнопка 2"));

	Возврат Элементы;
КонецФункции  

// функция-утилита для создания одного элемента
Функция СоздатьЭлемент(title, desc, type = "text", label)
    ...
    Действие.Вставить("link", "https://reddit.com");
