...
Функция ПолучитьТекстЗаказов(from_id) Экспорт
    ...
    Запрос.Текст = 
    "ВЫБРАТЬ
    |   Заказы.Пользователь КАК Пользователь,
    |   Заказы.Заказ КАК Заказ
    |ИЗ
    |   Справочник.ЗаказыПользователей КАК Заказы
    |ГДЕ
    |   Заказы.Пользователь = &Пользователь";
    Запрос.УстановитьПараметр("Пользователь", Строка(from_id));
    ...
    Пока Результат.Следующий() Цикл
        ...
        // Добавляем строку с позицией
        Текст = Текст + Строка(КоличЗаказов) + ". " + Символы.ПС;
    КонецЦикла;
    
    // 4. Если не было ни одной записи
    Если КоличЗаказов = 0 Тогда
        Возврат "У вас нет заказов.";
	КонецЕсли;                     
