...
Процедура СоздатьЗаказ(FromID, Email) Экспорт
	...
	Иначе
	    ...
	    Пока Результат.Следующий() Цикл         
			Док.ОбщаяСумма = ОбработатьСтрокуКорзины(Результат, Док);	        
	    КонецЦикла;
...
Функция ОбработатьСтрокуКорзины(Результат, Док)
	...
	Если СсылкаНом <> Неопределено Тогда  
		...
		Строка.Сумма = ЦенаТовара * Колво;  
		ОбщаяСумма   = ОбщаяСумма + ЦенаТовара * Колво;
	КонецЕсли;	
	Возврат ОбщаяСумма;
КонецФункции

// 2. Утилита: очистить корзину
Процедура ОчиститьКорзину(FromID) Экспорт
    Запрос = Новый Запрос;
    Запрос.Текст = 
    "ВЫБРАТЬ
