	...
	ИначеЕсли СтруктураДанных.Свойство("type") И СтруктураДанных.type = "message_new" Тогда
		...
		Если СтруктураДанных.Свойство("object") Тогда  
			Попытка      
				...
				Данные = Клавиатура.СформироватьКлавиатуру(ОбъектСообщение.text);
				
				Ответ.УстановитьТелоИзСтроки("ok", "UTF-8");         
				Если Данные.Свойство("template") Тогда
					message_send(random_id, peer_id, Данные.message, , Данные.template); 
				Иначе
...
Функция message_send(random_id = Неопределено, peer_id = Неопределено, 
					  message = Неопределено, keyboard = Неопределено, template = Неопределено)
